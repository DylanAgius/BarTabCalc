...
def __(mo):
    settings = mo.vstack(
        [
            ...
            num_guests := mo.ui.number(label="Number of guests", start=10, stop=1000),
            percent_drive := mo.ui.number(label="Percentage expected to drive", start=10, stop=90),
            percent_female := mo.ui.number(label="Percentage of women attending", start=10, stop=90),
            numhours := mo.ui.number(label="Hours you would like to tab to last", start=2, stop=12),
    ...
    return num_guests, percent_drive, percent_female, numhours,settings
...
# """
@app.cell
def simulate(num_guests,percent_drive, percent_female, numhours, weekday,stats,np,random,settings,pd):
   ...
   class itercalc:
        ...
        def __init__(self,data):
            ...
            self.weekday=data['schoolnight'].dropna().item()
   ...
   eventdata= {'num_guests': [int(num_guests.value)], #number of guests attending
   'percent_drive': [float(20/100)], #fraction driving
   'percent_female': [float(30/100)], #fraction female
   'num_hours': [int(3)],#number hours for the event (note: must be in multiples of 1 hour)
   'drinkprice':[5,6,8,9], # drink prices for available drinks
   'schoolnight': ['yes'] #specify weekend or weekday event. This alters the drinks per hour for each distribution
   }
   
   data = pd.DataFrame(dict([(key, pd.Series(value)) for key, value in eventdata.items()]))

     
   average_total_cost=str(np.ceil(runner(data).loop()))
...
@app.cell
def __(average_total_cost,num_guests, percent_drive, percent_female, numhours,mo):
    mo.md(
        f"""
        Your total tab value requred is $ **{average_total_cost}**
      
        """
    )  if all([num_guests.value>1, percent_drive.value>0, percent_female.value>0, numhours.value>1]) else None
    return
...
def __():
    ...
    return mo,np, plt, stats, random,pd
